...
export class TicketService {
  ...
  public getTicketList(start: Number, size: Number, ticket_order_oid: string, sort: string): Observable<{}> {
    const url = `${this.ticketsUrl}?start=${start}&size=${size}&ticket_order_oid=${ticket_order_oid}&sort=${sort}`;
    return this.http.get(url, this.options)
                    .map((response: Response) => response.json())
                    .catch((error: any) => Observable.throw(error.json() || 'Server error'));
  }
